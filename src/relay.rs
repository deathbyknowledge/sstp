  ...
  pub async fn start(&mut self) -> Result<(), Box<dyn Error>> {
    ...
    while let Some(socket) = incoming.next().await {
      ...
      tokio::spawn(async move {
        process_req(socket.unwrap(), rooms).await.expect("Error when processing request");
      });
    }
    Ok(())
  }
...
async fn process_req(socket: TcpStream, rooms: Arc<Mutex<HashMap<String, RoomInfo>>>) -> Result<(), Box<dyn Error>> {
